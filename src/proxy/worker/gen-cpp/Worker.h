...
class WorkerIf {
 public:
  virtual ~WorkerIf() {}
  virtual bool StartTask(const  ::TaskInfo& task_info) = 0;
};
...
typedef struct _Worker_StartTask_args__isset {
  _Worker_StartTask_args__isset() : task_info(false) {}
  bool task_info;
...
class WorkerClient : virtual public WorkerIf {
 public:
  ...
  bool StartTask(const  ::TaskInfo& task_info);
  void send_StartTask(const  ::TaskInfo& task_info);
  bool recv_StartTask();
 protected:
...
class WorkerMultiface : virtual public WorkerIf {
 ...
 public:
  bool StartTask(const  ::TaskInfo& task_info) {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->StartTask(task_info);
    }
    return ifaces_[i]->StartTask(task_info);
  }
